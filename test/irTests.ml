...
(** [ir_file_test name ~src ~out ~reference] tests ir generation
...
let ir_file_test name ~src ~out ~reference =
  ...
  let command = Printf.sprintf "./xic --irrun %s > %s.output" src src_no_ext in
...
(** [ir_file_tests path] compiles all test cases in [path] using
    [ir_file_test] *)
let ir_file_tests =
  map_file_tests_no_ixi ir_file_test ".irsol.nml"

(** [ir_file_test_cases] is a list of unit tests for ir generation files. *)
