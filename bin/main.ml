...
let lex_file_to_path input_file =
  ...
  let output_file_path = if !output_path = "" then file_prefix ^ ".lexed" 
    else !output_path ^ "/" ^ file_prefix ^ ".lexed" in
...
let speclist =
  [("-D", Arg.Tuple [Arg.Set_string output_path; Arg.Clear display_help], "Specify where to place generated diagnostic files.");
   ("--lex", Arg.Tuple [Arg.Set to_lex; Arg.Clear display_help], "Generate output from lexical analysis.");
...
let () = 
  let _ = 
    try
      Arg.parse speclist (fun f -> input_files := f :: !input_files) usage_msg;
    with
