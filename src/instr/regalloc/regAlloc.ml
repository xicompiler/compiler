...
(* let offset = Reg.Abstract.Table.create () in let allocate_fn =
   Trivial.allocate_fn in let body = allocate_fn ~offset fn in let temps
   = Int64.of_int (Hashtbl.length offset * 8) in Enter (temps, 0L) ::
   body *)
