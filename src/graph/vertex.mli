...
(** [Creators] allows to create arbitrary vertices *)
module Creators : sig
  ...
  (** [Make (Args)] is an [S2] with the same representation type and
      [Key] as [Args] *)
  module Make2 (Args : Params2) :
    S2 with type ('a, 'b) t := ('a, 'b) Args.t
  ...
  module Make (Args : Params) : S with type 'a t := 'a Args.t
end

(** [S2] represents a labeled vertex adjacent to weighted edges *)
module type S2 = sig
  include Creators.Params2
  include Creators.S2 with type ('a, 'b) t := ('a, 'b) t
...
(** [S] represents a labeled vertex adjacent to unweighted edges *)
module type S = sig
  include T1
  include S2 with type ('a, 'b) t := 'a t
end
